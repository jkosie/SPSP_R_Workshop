...
```{r}
ggplot(data = ma_data, mapping = aes(x = pvAvg, y = arithmeticTotal)) +
  geom_point(mapping = aes(color = group)) +
  geom_smooth(mapping = aes(color = group, linetype = group)) 
...
plot.data <- ma_data %>% 
  group_by(year, group) %>% 
  summarise(mean_ravens = mean(ravens), 
            sd = sd(ravens))

ggplot(plot.data, aes(x = as.factor(year), y = mean_ravens, fill = group)) +
  geom_bar(stat = "identity") +
  facet_wrap(~ group)
